...
module {
  ...
  // ── OldActor = NewActor of 20260618_000000_AddRoleFixAdminTier ───────────
  ...
  public func migration(old : OldActor) : NewActor {
    // Resync adminPrincipal into AccessControl state
    switch (old.adminPrincipal) {
      case (?admin) {
        let existingRole = old.accessControlState.userRoles.get(admin);
        switch (existingRole) {
          case (null) {
            old.accessControlState.userRoles.add(admin, #admin);
          };
          case (?_) {};
        };
