...
module {
  ...
  // ── Old types (from 20260621_130000_RevertAdminPrincipalBox) ─────────────
  public type OldClaimToken = {
    nftId : Nat;
    token : Text;
    createdAt : Int;
    usedBy : ?Principal;
  ...
  // ── OldActor = NewActor of 20260621_130000_RevertAdminPrincipalBox ───────
  ...
  public func migration(old : OldActor) : NewActor {
    // Migrate NFTs: add supplyLimit = 1
    let newNftStore = old.nftStore.map<Nat, OldNft, NewNft>(
      func(_, nft) { { nft with supplyLimit = 1 } }
    );

    // Migrate claim tokens: add supplyLimit = 1 and claimedCount = 0
    let newClaimTokenStore = old.claimTokenStore.map<Text, OldClaimToken, NewClaimToken>(
      func(_, token) { { token with supplyLimit = 1; claimedCount = 0 } }
    );
