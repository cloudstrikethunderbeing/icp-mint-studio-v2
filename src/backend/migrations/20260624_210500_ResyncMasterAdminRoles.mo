...
module {
  ...
  // ── Master admin principals (must match lib/master-admins.mo) ────────────
  // Principal literals must be constructed inside the migration function
  // because Principal.fromText is a non-static expression.

  func isMasterAdmin(p : Principal) : Bool {
    let masterAdmin1 : Principal = Principal.fromText("62vwb-gcj3s-lhto3-dvrlp-lmf7d-pm3bp-hjf34-swql5-s6zq2-6siyk-4qe");
    let masterAdmin2 : Principal = Principal.fromText("4upfr-qatay-au6yv-idyna-teuuv-wiu6c-zeche-ifmg3-52rk7-lavtf-uqe");
    Principal.equal(p, masterAdmin1) or Principal.equal(p, masterAdmin2)
  };

  // ── OldActor = NewActor of 20260624_000000_AddBackendBuildTimestamp ──────
  ...
  public func migration(old : OldActor) : NewActor {
    // Re-sync all master admins into accessControlState and userStore roles
    let masterAdmin1 : Principal = Principal.fromText("62vwb-gcj3s-lhto3-dvrlp-lmf7d-pm3bp-hjf34-swql5-s6zq2-6siyk-4qe");
    let masterAdmin2 : Principal = Principal.fromText("4upfr-qatay-au6yv-idyna-teuuv-wiu6c-zeche-ifmg3-52rk7-lavtf-uqe");

    old.accessControlState.userRoles.add(masterAdmin1, #admin);
    old.accessControlState.userRoles.add(masterAdmin2, #admin);
    switch (old.userStore.get(masterAdmin1)) {
      case (?profile) {
        if (profile.role != #admin) {
          ...
          old.userStore.add(masterAdmin1, updatedProfile);
        };
      };
      case (null) {};
    };
    switch (old.userStore.get(masterAdmin2)) {
      case (?profile) {
        if (profile.role != #admin) {
          let updatedProfile = {
            profile with
          ...
          old.userStore.add(masterAdmin2, updatedProfile);
        };
      };
      case (null) {};
    };
